...
} elseif ($accountType == 2) { // Student Leader
    $orgId = $_SESSION['org_id'];
    $sql = "
        SELECT org.org_name, college_department.department_name 
        FROM org 
        JOIN college_department ON org.department_id = college_department.id 
        WHERE org.id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $orgId);
    $stmt->execute();
    $stmt->bind_result($orgName, $departmentName);
    $stmt->fetch();
    $stmt->close();

    $accountName = $orgName;
    $accountDetails = 'Student Leader (' . $departmentName . ')';

} elseif ($accountType == 3) { // Office
    $officeId = $_SESSION['office_id'];
    $sql = "SELECT office_name FROM office WHERE id = ?";
    $stmt = $conn->prepare($sql);
        ...
        <div class="col-md-6">
            ...
            <div class="credentials-card">
                ...
                <div class="credentials-body">
                    ...
                    <p><strong>Account Type:</strong> <?php echo htmlspecialchars($accountDetails); ?></p>
